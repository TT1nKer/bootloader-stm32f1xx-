/**
  ******************************************************************************
  * @file    bootloader_main_automotive.c
  * @brief   Automotive-grade bootloader main program with enhanced reliability
  ******************************************************************************
  * @attention
  *
  * This file extends the original bootloader_main.c with automotive-grade features
...
/* Private defines -----------------------------------------------------------*/
#define AUTOMOTIVE_MODE_ENABLED       1  // Set to 1 to enable automotive features
...
static void Automotive_PreInit(void);
static void Automotive_PostInit(void);
static void Automotive_MainLoop_Process(void);

/* Private variables ---------------------------------------------------------*/
static uint32_t last_power_check = 0;
static uint32_t last_watchdog_feed = 0;
static bool automotive_features_initialized = false;

/* Main function -------------------------------------------------------------*/

/**
  * @brief  Automotive-grade bootloader main function
  * @retval int (never returns)
  */
int main(void)
{
    /* Automotive pre-initialization */
    Automotive_PreInit();
    ...
    /* Initialize OTA download service */
...
#if ENABLE_CAN_TRANSPORT
    OTA_TransportCan_Init();
#endif

    /* Automotive post-initialization */
    Automotive_PostInit();

    /* Enter upgrade mode */
    if (BSP_WatchdogAutomotive_Init(PROTECTION_LEVEL_AUTOMOTIVE) != HAL_OK)
    {
        Error_Handler();
    }
    ...
    while (1)
    {
        /* Process automotive features */
        Automotive_MainLoop_Process();

        /* Toggle LED every 500ms to indicate bootloader is running */
        ...
        /* Process transport polling */
        OTA_TransportBle_Poll();
        OTA_TransportRf_Poll();
#if ENABLE_CAN_TRANSPORT
        OTA_TransportCan_Poll();
#endif
    }
}
...
static bool TryPromoteStagedImage(OtaMetadata_t *meta, bool forced_upgrade)
{
    ...
    if (!BSP_Jump_IsApplicationValid(staged_address))
    {
        ...
        // Log automotive diagnostic event
        #if AUTOMOTIVE_MODE_ENABLED
        BSP_AutomotiveMeta_LogEvent(DIAG_EVENT_UNEXPECTED_RESET, OTA_ERROR_INVALID_IMAGE);
        #endif
    ...
    // Update automotive statistics
    #if AUTOMOTIVE_MODE_ENABLED
    BSP_AutomotiveMeta_UpdateStatistics();
    #endif
...
static void PrepareUpgradeWindow(OtaMetadata_t *meta, bool reset_window)
{
    ...
    if (modified)
    {
        ...
        // Enable upgrade mode in automotive watchdog
        #if AUTOMOTIVE_MODE_ENABLED
        BSP_WatchdogAutomotive_UpgradeMode_Enable();
        #endif
    }
}
...
/**
  * @brief  Automotive pre-initialization
  */
static void Automotive_PreInit(void)
{
    #if AUTOMOTIVE_MODE_ENABLED
    // Initialize power monitoring early for safety
    if (BSP_PowerMonitor_Init() != HAL_OK) {
        // Power monitoring failed, but continue with basic functionality
        automotive_features_initialized = false;
    } else {
        automotive_features_initialized = true;
    }
    #else
    automotive_features_initialized = false;
    #endif
...
/**
  * @brief  Automotive post-initialization
  */
static void Automotive_PostInit(void)
{
    #if AUTOMOTIVE_MODE_ENABLED
    if (automotive_features_initialized) {
        // Check automotive metadata compatibility
        if (!BSP_AutomotiveMeta_IsCompatible()) {
            // Migrate from legacy metadata if needed
            BSP_AutomotiveMeta_MigrateFromLegacy();
        }

        // Log power cycle event
        BSP_AutomotiveMeta_LogEvent(DIAG_EVENT_POWER_CYCLE, 0);
    }
    #endif
}

/**
  * @brief  Automotive main loop processing
  */
static void Automotive_MainLoop_Process(void)
{
    #if AUTOMOTIVE_MODE_ENABLED
    ...
    #else
    /* Legacy watchdog feeding for backward compatibility */
    static uint32_t last_watchdog_feed_legacy = 0;
    uint32_t current_time = HAL_GetTick();

    if (current_time - last_watchdog_feed_legacy >= 500)
    {
        BSP_Watchdog_Feed();
        last_watchdog_feed_legacy = current_time;
    }
    #endif
}
...
void Error_Handler(void)
{
    ...
    while (1)
    {
        ...
        /* Feed watchdogs to prevent reset loop */
        #if AUTOMOTIVE_MODE_ENABLED
        if (automotive_features_initialized) {
            BSP_WatchdogAutomotive_FeedSystem();
        } else {
